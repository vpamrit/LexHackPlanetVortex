...
class Ball(pygame.sprite.Sprite):
    def __init__(self, x, y, dx, dy, r, angle):
        ...
        self.angle = 20
        self.speed = 0.2 #  cap for the self.Vangle quantity. Terminal Velocity?
        self.draw_width = 4
        self.stop = 1
        self.Aangle = 0
        self.Vangle = 0
    ...
    def grav(self):#IMPLEMENT TAKE OFF
        if(self.totalr>self.r+earth.r or self.dr >0):#off earth
            self.totalr += self.dr
            
        else:#on earth
            self.ddr = 0
            self.dr = -.005
            if(abs(self.Vangle) >=  0.15):
                #self.dr = 0.3
                #self.totalr += self.dr
                self.ddr = 0.0051
        #print self.totalr
    ...
    def clickRotate(self):
        ...
        if(self.totalr < earth.r + self.r+2):
            ...
            if(key[K_LEFT]and self.Aangle <0.0008):
                self.Aangle += 0.000035
            self.SLOW = 1.012#on earth friction
        else:
            self.SLOW = 1.005#off earth friction/air resistance
